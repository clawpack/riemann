...
! ============================================================================
    ...
    use geoclaw_module, only: g => grav, pi, earth_radius, dry_tolerance
    ...
    use multilayer_module, only: lapack_eigen, linearized_eigen, vel_diff_eigen
...
!         endif
        ...
        if (.not.(info == 0)) then
            ...
            do m=1,2
                do j=1,3
                    print "(2d16.8)", qr(3*(m-1)+j,i-1) / rho(m), ql(3*(m-1)+j,i) / rho(m)
                end do
            end do
