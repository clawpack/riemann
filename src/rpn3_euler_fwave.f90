...
!
! Description: Riemann F-wave solver for the 3D Euler Equations
!
! Method: Riemann solver for 3D Euler (neutral fluid) with F-Wave jumps.
...
!         mx <INTEGER>     : number of grid cells
...
SUBROUTINE rpn3(ixyz,maxm,meqn,mwaves,maux,mbc,mx,ql,qr,auxl,auxr,fwave,s,amdq,apdq)
  ...
  REAL(kind=8), DIMENSION(5) :: delta,beta
  ...
  INTEGER :: i,j,m,mu,mv,mw,mws,il,ir
  ...
  IF (maxmrp < maxm+mbc)THEN
     WRITE(6,*) 'need to increase maxmrp in rpn3_neutral_fwave.f90'
  ...
  !   find beta(1) thru beta(5), the coefficients of the 5 eigenvectors:
  !---------------------------------------------------------------------------------
  DO i = 2-mbc, mx+mbc
     ...
     delta(2) = (ql(mu,i)*ql(mu,i)/ql(1,i)+pl) &
          - (qr(mu,i-1)*qr(mu,i-1)/qr(1,i-1)+pr)
     delta(3) = (ql(mu,i)*ql(mv,i)/ql(1,i)) - (qr(mu,i-1)*qr(mv,i-1)/qr(1,i-1))
     delta(4) = (ql(mu,i)*ql(mw,i)/ql(1,i)) - (qr(mu,i-1)*qr(mw,i-1)/qr(1,i-1))
     delta(5) = ((ql(5,i)+pl)*ql(mu,i)/ql(1,i)) &
          - ((qr(5,i-1)+pr)*qr(mu,i-1)/qr(1,i-1))

     beta(4) = g1a2(i) * (euv(i)*delta(1) &
          + u(i)*delta(2) + v(i)*delta(3) + w(i)*delta(4) - delta(5))
     beta(2) = delta(3) - v(i)*delta(1)
     beta(3) = delta(4) - w(i)*delta(1)
     beta(5) = (delta(2) + (a(i)-u(i))*delta(1) - a(i)*beta(4)) / (2.d0*a(i))
     ...
     !------------------------------------------------------------------------------
     fwave(1,1,i)  = beta(1)
     fwave(mu,1,i) = beta(1)*(u(i)-a(i))
     fwave(mv,1,i) = beta(1)*v(i)
     fwave(mw,1,i) = beta(1)*w(i)
     fwave(5,1,i)  = beta(1)*(enth(i) - u(i)*a(i))
     s(1,i) = u(i)-a(i)
     
     fwave(1,2,i)  = beta(4)
     fwave(mu,2,i) = beta(4)*u(i)
     fwave(mv,2,i) = beta(4)*v(i) + beta(2)
     fwave(mw,2,i) = beta(4)*w(i) + beta(3)
     fwave(5,2,i)  = beta(4)*0.5d0*u2v2w2(i) + beta(2)*v(i) + beta(3)*w(i)
     s(2,i) = u(i)
     
     fwave(1,3,i)  = beta(5)
     fwave(mu,3,i) = beta(5)*(u(i)+a(i))
     fwave(mv,3,i) = beta(5)*v(i)
     fwave(mw,3,i) = beta(5)*w(i)
     fwave(5,3,i)  = beta(5)*(enth(i)+u(i)*a(i))
     s(3,i) = u(i)+a(i)
  END DO
  ...
  ELSE
     ...
     !------------------------------------------------------------------------------
     ! Compute flux differences amdq and apdq.
     !   First compute amdq as sum of fwave for left going waves.
     ...
     DO i = 2-mbc, mx+mbc
        ...
        ENDIF
        amdq(:,i) = sfract*fwave(:,1,i)/s(1,i)
           
        !---------------------------------------------------------------------------
        ! Check 2-wave
        !---------------------------------------------------------------------------
        IF (s(2,i) >= 0.d0)CYCLE ! 2-,3- and 4- waves are rightgoing
        amdq(:,i) = amdq(:,i) + fwave(:,2,i)
        ...
        ELSE
           ...
           CYCLE
        ENDIF
        amdq(:,i) = amdq(:,i) + sfract*fwave(:,3,i)/s(3,i)
     END DO
     
     !---------------------------------------------------------------------------
     ! Compute the remaining right-going flux differences:
     ! df = SUM fwave is the total flux difference and apdq = df - amdq
     !---------------------------------------------------------------------------
     DO m=1,meqn
        DO i = 2-mbc, mx+mbc
           df = 0.d0
