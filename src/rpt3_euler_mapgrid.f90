...
!          bpasdq <REAL>   : right-going flux differences
!
! Adapted from rpt3_euler.f90 in $CLAWHOME/riemann/src
!-----------------------------------------------------------------------------------
SUBROUTINE rpt3(ixyz,icoor,imp,maxm,meqn,mwaves,maux,mbc,mx,ql,qr,aux1,aux2,aux3,&
     asdq,bmasdq,bpasdq)
  ...
  DO i = 2-mbc, mx+mbc
     IF (qr(1,i-1) <= 0.d0 .OR. ql(1,i) <= 0.d0) THEN
        ...
        IF (ixyz == 1) WRITE(6,*) '*** (rpt3) rho <= 0 in x-sweep at ',i
        IF (ixyz == 2) WRITE(6,*) '*** (rpt3) rho <= 0 in y-sweep at ',i
        IF (ixyz == 3) WRITE(6,*) '*** (rpt3) rho <= 0 in z-sweep at ',i
     ...
     IF (i>=0 .AND. i<=mx .AND. asqrd <= 0.d0) THEN
        IF (ixyz == 1) WRITE(6,*) '*** (rpt3) a**2 <= 0 in x-sweep at ',i
        IF (ixyz == 2) WRITE(6,*) '*** (rpt3) a**2 <= 0 in y-sweep at ',i
        IF (ixyz == 3) WRITE(6,*) '*** (rpt3) a**2 <= 0 in z-sweep at ',i
  ...
  DO i = 2-mbc, mx+mbc
     ...
     IF(icoor == 2)THEN
        areaRatioMinus = aux2(iratio,i-2+imp,2)
        areaRatioPlus  = aux3(iratio,i-2+imp,2)
     ELSEIF(icoor == 3)THEN
        areaRatioMinus = aux2(iratio,i-2+imp,2)
        areaRatioPlus  = aux2(iratio,i-2+imp,3)
     ENDIF
     ...
     beta(4) = g1a2(i)*(euv(i)*asdq(1,i) &
          + u(i)*asdq(mu,i) + v(i)*asdq(mv,i) + w(i)*asdq(mw,i) - asdq(5,i))
     beta(5) = ( (a(i)-vn)*asdq(1,i) - a(i)*beta(4) &
          + nx*asdq(mu,i) + ny*asdq(mv,i) + nz*asdq(mw,i) )/(2.d0*a(i))
     beta(1) = asdq(1,i) - beta(4) - beta(5)
     
     SELECT CASE(TRIM(nonsingularNormal))
     CASE("x")
        beta(2) = ( (v(i)-vn*ny)*asdq(1,i) + nx*ny*asdq(mu,i) &
             + (ny**2-1)*asdq(mv,i) + ny*nz*asdq(mw,i) )/nx
        beta(3) = (-(w(i)-vn*nz)*asdq(1,i) - nx*nz*asdq(mu,i) &
             - ny*nz*asdq(mv,i) - (nz**2-1)*asdq(mw,i) )/nx
     CASE("y")
        beta(2) = (-(u(i)-vn*nx)*asdq(1,i) - (nx**2-1)*asdq(mu,i) &
             - ny*nx*asdq(mv,i) - nx*nz*asdq(mw,i) )/ny
        beta(3) = ( (w(i)-vn*nz)*asdq(1,i) + nx*nz*asdq(mu,i) &
             + ny*nz*asdq(mv,i) + (nz**2-1)*asdq(mw,i) )/ny
     CASE("z")
        beta(2) = ( (u(i)-vn*nx)*asdq(1,i) + (nx**2-1)*asdq(mu,i) &
             + nx*ny*asdq(mv,i) + nz*nx*asdq(mw,i) )/nz
        beta(3) = (-(v(i)-vn*ny)*asdq(1,i) - nx*ny*asdq(mu,i) &
             - (ny**2-1)*asdq(mv,i) - nz*ny*asdq(mw,i) )/nz
     END SELECT
     ...
     DO mws = 1,mwaves
        IF (sb(mws) < 0.d0)THEN
           ...
           bmasdq(:,i) = bmasdq(:,i) + sb(mws)*waveb(:,mws)
        ELSE
           waveb(:,mws) = waveb(:,mws)*areaRatioPlus
           bpasdq(:,i) = bpasdq(:,i) + sb(mws)*waveb(:,mws)
        END IF
     END DO
  END DO
