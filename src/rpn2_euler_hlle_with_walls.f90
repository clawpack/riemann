...
! Same as rpn2_euler_hlle except that
! this solver takes into account internal reflecting boundaries (walls) within the domain,
! making it possible to solve problems like flow over a step.
! The aux array contains the geometry: where it is equal to zero, a solid object exists.
...
!
    ...
    do i=2-mbc,mx+mbc
        ...
        if( (auxr(1,i-1) .eq. 0.d0) .and. (auxl(1,i) .eq. 1.d0) ) then
            ! Reflecting boundary to left
            qr(1,i-1) = ql(1,i)
            qr(mu,i-1) = -ql(mu,i)
            qr(mv,i-1) = ql(mv,i)
            qr(4,i-1) = ql(4,i)
         else if( (auxl(1,i) .eq. 0.d0) .and. (auxr(1,i-1) .eq. 1.d0) ) then
